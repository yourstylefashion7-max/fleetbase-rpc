...
trait HasApiModelBehavior
{
    ...
    public function remove($id)
    {
        ...
        if ($this->isColumn('company_uuid')) {
            $record->where($this->qualifyColumn('company_uuid'), $companyUuid);
        }
    ...
    public function bulkRemove($ids = [])
    {
        ...
        return $this->getConnection()->transaction(function () use ($ids) {
            ...
            $countQuery = clone $records;
            if (method_exists($countQuery, 'lockForUpdate')) {
                $countQuery->lockForUpdate();
            }

            $count = $countQuery->count();
            if ($count !== count($ids)) {
                throw (new \Illuminate\Database\Eloquent\ModelNotFoundException())->setModel(static::class, $ids);
            }
