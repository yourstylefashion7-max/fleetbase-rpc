...
class AuthController extends Controller
{
    ...
    public function switchOrganization(SwitchOrganizationRequest $request)
    {
        ...
        $user                 = $request->user();
        $previousOrganization = $user->company_uuid;
        ...
        // Organization switching is the audited cross-tenant membership lookup.
        // Preserve soft-delete filtering so revoked memberships never authorize.
        $hasActiveMembership = CompanyUser::withoutGlobalScope(CompanyScope::class)
            ->where(['user_uuid' => $user->uuid, 'company_uuid' => $nextOrganization])
            ->where('status', 'active')
            ->exists();
        $hasActiveCompany = Company::withoutGlobalScope(CompanyScope::class)
            ->active()
            ->where('uuid', $nextOrganization)
            ->exists();
        ...
        TenantContext::run($nextOrganization, function () use ($user, $nextOrganization) {
        ...
        if (config('activitylog.enabled', true)) {
            activity('security')
                ->causedBy($user)
                ->performedOn($user)
                ->event('organization.switched')
                ->withProperties([
                    'from_company_uuid' => $previousOrganization,
                    'to_company_uuid'   => $nextOrganization,
                ])
                ->log('Organization session switched');
        }
    ...
    {
        try {
            $company = Company::active()->where('public_id', $request->input('next'))->firstOrFail();
            ...
            $result = TenantContext::run($company->uuid, function () use ($company, $user) {
