...
class CallWebhookJob implements ShouldQueue
{
    ...
    {
        $companyUuid = data_get($this->meta, 'company_uuid');
        if (empty($companyUuid)) {
            throw new \LogicException('Queued webhooks require an explicit tenant company UUID.');
        }

        return TenantContext::run(
            $companyUuid,
            fn () => $this->handleInTenant(),
            syncSession: false
        );
    }

    protected function handleInTenant()
    {
