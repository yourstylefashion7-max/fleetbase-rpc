...
class SendResourceLifecycleWebhook implements ShouldQueue
{
    ...
    {
        $companyUuid = $event->companySession ?? null;
        if (empty($companyUuid)) {
            logger()->error('Unable to process lifecycle webhook without a company context.');

            return;
        }

        $sessionKeys = ['api_credential', 'api_key', 'api_secret', 'api_environment', 'is_sandbox', 'company', 'user'];
        $snapshot    = collect($sessionKeys)->mapWithKeys(fn ($key) => [
            $key => ['exists' => session()->has($key), 'value' => session()->get($key)],
        ])->all();

        try {
            return TenantContext::run(
                $companyUuid,
                fn () => $this->handleInTenant($event),
                syncSession: false
            );
        } finally {
            foreach ($snapshot as $key => $state) {
                if ($state['exists']) {
                    session()->put($key, $state['value']);
                } elseif (method_exists(session(), 'forget')) {
                    session()->forget($key);
                } else {
    ...
    protected function handleInTenant($event)
    {
        ...
        foreach ($webhooks as $webhook) {
            ...
            $signingCredential = ApiCredential::where('uuid', $webhook->api_credential_uuid)
                ->where('company_uuid', $webhook->company_uuid)
                ->first();

            if (!$signingCredential || empty($signingCredential->secret)) {
                logger()->error('Unable to dispatch lifecycle webhook without an active endpoint signing credential.', [
                    'company_uuid'                  => $webhook->company_uuid,
                    'webhook_uuid'                  => $webhook->uuid,
                    'signing_api_credential_uuid'   => $webhook->api_credential_uuid,
                ]);
            ...
            try {
                ...
                WebhookCall::create()
                    ...
                    ->url($webhook->url)
                    ->payload($payload)
                    ->useSecret($signingCredential->secret)
                    ->dispatch();
